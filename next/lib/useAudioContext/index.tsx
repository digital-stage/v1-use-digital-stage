...
const createBuffer = (sampleRate?: number): IAudioContext => {
    ...
    context.listener.positionZ.setValueAtTime(0, context.currentTime);
...
const startAudioContext = async (audioContext: IAudioContext, audio: HTMLAudioElement): Promise<void> => {
...
export const AudioContextProvider = (props: { children: React.ReactNode }): JSX.Element => {
    ...
    useEffect(() => {
        ...
        setStarted(false);
        if (sampleRate)
            report("Using sample rate of " + sampleRate);
        ...
        return () => {
            ...
            standardizedAudioContext.close()
                .catch(error => reportWarning(error));
