...
const MediasoupProvider = (props: {
          ...
          consume(producer)
            .then((consumer) => {
              if (isAudioProducer(producer)) {
                dispatch(
                  allActions.stageActions.client.addAudioConsumer(consumer)
                );
              } else {
                dispatch(
                  allActions.stageActions.client.addVideoConsumer(consumer)
                );
              }
              return undefined;
            })
  ...
  const shareVideo = useCallback(
    (tracks: MediaStreamTrack[]) => {
      return Promise.all(
        tracks.map((track) => produce(track))
      ).then((localProducers) =>
  ...
  useEffect(() => {
    if (ready) {
      if (receiveVideo) {
        ...
        producersWithoutConsumer.forEach((producer) =>
          createConsumer(producer)
        );
        consumersWithoutProducer.forEach((consumer) =>
          removeConsumer(consumer, 'video')
        );
      } else {
